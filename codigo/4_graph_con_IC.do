...
foreach num of numlist 22/23 { 
    use "$rutapro\ech`num'_procesada.dta", clear
    keep bc_correlat bc_percap_iecon bc_pesoan
    duplicates drop
    xtile cuartil = bc_percap_iecon [pw=bc_pesoan], nq(4)
    keep bc_correlat cuartil
    save "$rutapro\ech`num'_cuartil.dta", replace
}

use "$rutapro\ech22_cuartil.dta", clear
append using "$rutapro\ech23_cuartil.dta"
bysort bc_correlat:  gen dup = cond(_N==1,0,_n)
bysort bc_correlat: egen cuartil_pc = max(cuartil)
keep bc_correlat cuartil_pc
duplicates drop
save "$rutapro\cuartil_aux.dta", replace
...
foreach tipo in "a" "m" {
	...
	foreach num of numlist 18/23 {
		...
		* cargar los datos: cuando "a", datos anuales, cuando "m", datos mensuales
		if "`tipo'"=="a" {
			use "$rutapro\ech`num'_procesada.dta", clear
            xtile cuartil_pc = bc_percap_iecon [pw=bc_pesoan], nq(4)
			if `num' <= 21 {
				* hasta 2021 usamos pesos "normales"
				svyset [pweight=bc_pesoan]
			}
			else { 
				merge m:1 bc_correlat bc_anio using "$rutapro\pesos_anuales_`num'.dta"
				drop _merge 
				* desde 2022 usamos matriz de pesos replicados
				svyset [pweight=bc_pesoan], bsrweight(wr*)
			}
		}
		else {
			if `num' <= 21 {
			...
			use "$rutapro\ech_mens_`num'_procesada.dta", clear
            merge m:1 bc_correlat using "$rutapro\cuartil_aux.dta"
			drop _merge
			merge m:1 bc_correlat bc_anio bc_mes using "$rutapro\pesos_replicados_22_23.dta" 
			drop if _merge != 3
			drop _merge
			svyset [pweight=bc_pesomen], bsrweight(wr*)
		}
...
cap g pet=(bc_pe3>13 & bc_pe3!=.)
cap drop pea
gen pea=.
replace pea=0 if pet==1
replace pea=1 if (bc_pobp>1 & bc_pobp<6)& pet==1
cap drop  desocu
g  desocu=.
replace  desocu=0 if pea==1 
replace  desocu=1 if bc_pobp>2 & bc_pobp<6

cap drop ocup
gen ocup=.
replace ocup=0 if pet==1
replace ocup=1 if bc_pobp==2 & pet==1

gen bc_horas_1=0
	cap replace bc_horas_1 = bc_horas_sp_1 if bc_anio<2001
	cap replace bc_horas_1 = bc_horas_hab_1 if bc_anio>2000
	cap replace bc_horas_1 =. if ocup!=1
	
recode bc_register (-9=.) (2=0), gen(register_alt)

gen bc_noregister = 1 - register_alt
...
if "`top'" == "des" {
    ...
    gen descvar = bc_noregister
}
else if "`top'" == "hor" {
    gen descvar = bc_horas_1
}
...
graph export "$rutares\`top'.png", name( `top'_graph) replace
...
qui twoway (rarea `X'_lb_a `X'_ub_a anio_a, color(blue%10) lwidth(none)) ///
       ...
       title("`label1'`label2'") ///
	   name(`X'_graph, replace)
graph export "$rutares\`X'.png", name(`X'_graph) replace
	   }

qui graph combine  `top'fem_graph  `top'masc_graph, rows(1) cols(2) xsize(15) ysize(6) name(combined_graph, replace)
graph export "$rutares\`top'_sexo.png", name(combined_graph) replace
...
graph export "$rutares\`top'_mvd_int.png", name(combined_graph) replace
...
graph export "$rutares\`top'_edad.png", name(combined_graph) replace
...
graph export "$rutares\`top'_educ.png", name(combined_graph) replace
...
graph export "$rutares\`top'_cuartil.png", name(combined_graph) replace
