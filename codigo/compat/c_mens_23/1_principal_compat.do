...
destring e197_1, replace force
destring e51_6a, replace force
destring e51_6b, replace force
destring e51_11, replace force
destring e201_1c, replace force
destring e201_1a, replace force
destring e201_1b, replace force
destring e201_1d, replace force
destring e215_1, replace force
destring e218_1, replace force
destring e221_1, replace force
...
/* Tamaño del establecimiento // no está f77 en seguimiento
g bc_pf081=-9
...
bysort bc_correlat bc_nper bc_pobp: egen rama_aux = max(f72_2)
gen rama_aux2 = f72_2
replace rama_aux2 = rama_aux if f72_2==.
tab rama_aux2 if bc_pobp == 2

g bc_rama=-9
	replace bc_rama=1 if rama_aux2>0000 & rama_aux2<1000
	replace bc_rama=2 if rama_aux2>1000 & rama_aux2<3500
	replace bc_rama=3 if rama_aux2>3500 & rama_aux2<3700
	replace bc_rama=4 if rama_aux2>4000 & rama_aux2<4500
	replace bc_rama=5 if (rama_aux2>=4500&rama_aux2<4900)|(rama_aux2>=5500&rama_aux2<5700)
	replace bc_rama=6 if (rama_aux2>=4900&rama_aux2<5500)|(rama_aux2>=5800&rama_aux2<6400)
	replace bc_rama=7 if rama_aux2>=6400&rama_aux2<8300
	replace bc_rama=8 if rama_aux2>=8300&rama_aux2<9910 | (rama_aux2>= 3700 & rama_aux2<=3900)
	
drop rama_aux*
*tab bc_rama if bc_pobp == 2
* todavía tengo un 1,6% con -9 en rama
* tab f72_2 if bc_pobp==2 & bc_rama == -9 
* pero acá son solo 6, no se qué pasa
...
* Causas por las que dejó último trabajo // no está en 2023
*g bc_pf34=f122
