...
gen bc_asalariados = (bc_pg11p+bc_pg12p+bc_pg14p+bc_pg15p+bc_pg16p+bc_pg17p+bc_pg21p+bc_pg22p+bc_pg24p+bc_pg25p+bc_pg26p+bc_pg27p+bc_pg11t+bc_pg12t+bc_pg14t+bc_pg15t+bc_pg16t+bc_pg17t)*bc_ipc
...
mvencode bc_pg51o bc_pg52o bc_pg31o bc_pg33o bc_pg41o bc_pg43o bc_pg91o bc_pg93o bc_pg71o bc_pg73o bc_pg60o bc_pg80o bc_pg60o_cpsl bc_pg60o_cpcl, mv(0) override
...
* no puedo distinguir local en la ocupación secundaria y bc_cpropiacl/sl suman de ocupación principal y secundaria
* voy a separar esta en principal cl sl, secundaria todo junto y total todo junto
...
* cuenta propia en ocupación principal con local
cap drop bc_cpropiacl_p
gen bc_cpropiacl_p=0
replace bc_cpropiacl_p=(bc_pg41p+bc_pg43p)*bc_ipc
cap drop cpropiaclph
egen cpropiaclph=sum(bc_cpropiacl_p) if bc_pe4!=7, by(bc_correlat)

* cuenta propia en ocupación principal sin local
cap drop bc_cpropiasl_p
gen bc_cpropiasl_p=0
replace bc_cpropiasl_p=(bc_pg31p+bc_pg33p)*bc_ipc
cap drop cpropiaslph
egen cpropiaslph=sum(bc_cpropiasl_p) if bc_pe4!=7, by(bc_correlat)

* cuenta propia con o sin local, ocupación principal y secundaria
cap drop bc_cpropia
gen bc_cpropia=0
replace bc_cpropia=(bc_pg31p+bc_pg33p+bc_pg41p+bc_pg43p+bc_pg91o+bc_pg93o)*bc_ipc
cap drop cpropiah
egen cpropiah=sum(bc_cpropia) if bc_pe4!=7, by(bc_correlat)
...
replace bc_principal=bc_patrones+bc_cpropia+bc_cooperat if bc_pf41>2 & bc_pf41<7 

capture drop bc_ing_lab
gen bc_ing_lab=0
replace bc_ing_lab= bc_as_privados+ bc_as_publicos+ bc_patrones+ bc_cpropia + bc_cooperat +bc_as_otros+bc_as_agropec+bc_cp_agropec+ /* 
...
replace benef_ocup_secund=bc_pg13o+bc_pg23o+bc_pg92o+bc_pg72o if bc_anio>1990
...
cap drop val_loc
gen val_loc=0
replace val_loc=(bc_pg14) if /*bc_pe4==1*/ n_miembro==1 // en 2021 hay problemas con la relación de parentesco y algunos hogares no tienen jefe
...
replace bc_ht11_sss_corr=((ing_labh+ing_caph+cuota_otrohog_h)/bc_ipc +beneficiosh+jub_penh+jub_peneh+ val_loch+otrosh+transf_hogh) if /*bc_pe4==1*/ n_miembro==1 // en 2021 hay problemas con la relación de parentesco y algunos hogares no tienen jefe
